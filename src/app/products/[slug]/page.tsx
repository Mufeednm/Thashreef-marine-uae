...
import { listStorefrontProducts } from "@/application/catalog/catalog-service";
...
export async function generateMetadata({ params }: ProductPageProps): Promise<Metadata> {
  ...
  const product = (await listStorefrontProducts(createDemoStoreRepository())).find(
...
export default async function ProductPage({ params }: ProductPageProps): Promise<ReactElement> {
  const { slug } = await params;
  const products = await listStorefrontProducts(createDemoStoreRepository());

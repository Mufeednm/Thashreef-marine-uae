...
export function CategoryNavigation({ categories, selectedCategoryId, selectCategory }: CategoryNavigationProps): ReactElement {
  ...
  return <nav aria-label="Product categories" className="relative border-t border-slate-100 bg-white/95" onMouseLeave={() => setOpenId(null)}>
    <div className="mx-auto flex max-w-[1480px] items-center gap-1 overflow-x-auto px-4 py-2 sm:px-6 [scrollbar-width:none]">
      {categories.map((category, index) => <div className="shrink-0" key={category.id} onMouseEnter={() => setOpenId(category.id)}>
        ...
        {openId === category.id && category.children.length > 0 ? <div className="absolute start-4 top-full z-50 mt-1 w-[min(28rem,calc(100vw-2rem))] origin-top rounded-2xl border border-slate-200 bg-white p-2 shadow-xl shadow-slate-950/15 motion-safe:animate-[menu-in_200ms_ease-out] sm:start-6" id={`category-submenu-${category.id}`} role="menu">

...
function RelatedProductCard({ product }: { product: Product }): ReactElement {
  const { locale } = useLocale();
  const name = localizedProductName(product, locale);
  return <article className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg"><Link aria-label={`View ${name}`} className="block" href={`/products/${product.slug}`}><div className="aspect-square bg-gradient-to-br from-sky-50 to-slate-100 p-5"><Image alt={name} className="h-full w-full object-contain" height={420} src={product.imageUrl} unoptimized={product.imageUrl.startsWith("/")} width={420} /></div><div className="p-4"><p className="text-xs font-black tracking-[0.14em] text-slate-400 uppercase">{product.brand}</p><h3 className="mt-2 text-sm font-black leading-5 text-[#0a2540]">{name}</h3><p className="mt-3 text-base font-black text-[#0e7490]">{formatAedFromCents(product.priceAedCents)}</p></div></Link></article>;
}

function localizedProductName(product: Product, locale: "ar" | "en"): string { return locale === "ar" && product.nameAr ? product.nameAr : product.name; }
function localizedProductDescription(product: Product, locale: "ar" | "en"): string { return locale === "ar" && product.descriptionAr ? product.descriptionAr : product.description; }

      ...
      <div className="mx-auto max-w-[1280px] px-4 py-7 sm:px-6 sm:py-10">
        ...
        <article className="mt-6 grid overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm lg:grid-cols-[1.05fr_0.95fr]">
          ...
          <div className="p-6 sm:p-10">
            ...
            <div className={`mt-7 rounded-2xl px-4 py-3 text-sm font-bold ${product.stockQuantity > 0 ? "bg-emerald-50 text-emerald-800" : "bg-rose-50 text-rose-800"}`}>{product.stockQuantity > 0 ? `${product.stockQuantity} units available — ready to order` : "Currently unavailable"}</div>
            <button className="mt-5 min-h-12 w-full rounded-full bg-[#f97316] px-6 text-sm font-black text-white transition hover:bg-[#c2410c] disabled:cursor-not-allowed disabled:bg-slate-300" disabled={product.stockQuantity === 0} onClick={addToCart} type="button">{product.stockQuantity === 0 ? "Unavailable" : added ? "Added to cart" : "Add to cart"}</button>

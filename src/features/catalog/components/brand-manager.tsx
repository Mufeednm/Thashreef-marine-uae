...
function BrandFields({ brand, compact = false, errors }: { brand?: Brand; compact?: boolean; errors?: CreateCategoryActionState["fieldErrors"] }): ReactElement {
  ...
  return <div className={layout}>
    ...
    <label className="block space-y-2"><span className="text-sm font-semibold text-slate-700">Brand name (Arabic)</span><input className="min-h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm outline-none focus:border-[#0e568f] focus:bg-white" defaultValue={brand?.nameAr ?? ""} dir="rtl" name="nameAr" placeholder="اسم العلامة بالعربية" /><Error message={errors?.nameAr?.[0]} /></label>
